...
class UsersAction extends BackAction{
    ...
    public function add(){
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            //根据表单提交的POST数据创建数据对象
			//dump(strtotime('2037-12-31'));
			//dump($_POST['viptime']);dump(strtotime($_POST['viptime']));exit;
            ...
            if($UserDB->create()){				
                ...
                $user_id = $UserDB->add();
                if($user_id){
					//增加权限分配 by zcb 20140621
					M('myfun')->where(array('users'=>$user_id))->delete();
					foreach($_POST as $key=>$value){
						if(substr($key,0,5) == "myfun"){
							$myfunarr = array();
							$myfunarr['users'] = $user_id;
							$myfunarr['funtype'] = $value;
							M('myfun')->add($myfunarr);
						}
					}
					
					//$UserDB->where(array('id'=>$user_id))->save(array('viptime'=>strtotime('2099-12-31')));
			...
			$allfun = M('function')->Distinct(true)->field('funtype')->select();
			$this->assign('allfun',$allfun);
			//dump($allfun);exit;
            $this->display();
        }
    }
    ...
    public function edit(){
        ...
        if(isset($_POST['dosubmit'])) {
			...
			$this->assign('allfun',$allfun);
			//dump($allfun);exit;
			$myfun = M('myfun')->where(array('users'=>$id))->select();
			foreach($myfun as $onefun){
				$myfunfun[$onefun['funtype']] = "checked";
			}
			$this->assign('myfun',$myfunfun);
			//dump($myfunfun);exit;
